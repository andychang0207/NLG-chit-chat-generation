                    ...
                    if 'beginning' in turn:
                        for candidate in turn['beginning']:
                            if candidate['label'] == "good":
                                seq = "<|context|>" + history + " <|endofcontext|> " + belief + "<|action|> " + "chit-chat in beginning, " +", ".join(action) + " <|endofaction|> " + "<|response|> " + utterance + " <|endofresponse|> " + "<|chitchat|> " + candidate['candidate'] + " <|endofchitchat|>"
                                processed_data.append({"text":"<|endoftext|>"+seq+"<|endoftext|>"})
                    if 'end' in turn:
                        for candidate in turn['end']:
                            if candidate['label'] == "good":
                                seq = "<|context|>" + history + " <|endofcontext|> " + belief + "<|action|> " + "chit-chat in end, " + ", ".join(action) + " <|endofaction|> " + "<|response|> " + utterance + " <|endofresponse|> " + "<|chitchat|> " + candidate['candidate'] + " <|endofchitchat|>"
                                processed_data.append({"text":"<|endoftext|>"+seq+"<|endoftext|>"})
                    # no chit-chat case
                    if ('beginning' not in turn and 'end' not in turn) or (len(turn['beginning']) == 0 and len(turn['end']) == 0):
                        seq = "<|context|>" + history + " <|endofcontext|> " + belief + "<|action|> " +", ".join(action) + " <|endofaction|> "+ "<|response|> " + utterance + " <|endofresponse|> " + "<|chitchat|> " + " <|endofchitchat|>"
                        processed_data.append({"text":"<|endoftext|>"+seq+"<|endoftext|>"})
                    # *****----- collect chit-chat and generate sequence input ------*****
                    # *****----- record history ------*****
    ...
    for dialogues_path in dialogues_list:
        ...
        for dialogue in dialogues:
            ...
            for idx,turn in enumerate(dialogue['turns']):
                ...
                if speaker == " <|user|> ":
                    processed_data.append({'text':"<|context|>"+history+" <|endofcontext|>",'response':"<|response|>"+dialogue['turns'][idx+1]['utterance']+"<|endofresponse|>"})
        progress_bar.update(1)
    output_name = os.path.join(args.output_dir,args.data_name+".jsonl")
    output_json = os.path.join(args.output_dir,args.data_name+".json")
    with jsonlines.open(output_name,mode="w") as writer:
